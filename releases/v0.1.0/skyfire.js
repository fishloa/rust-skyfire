...
export class SkyfirePlayer {
  ...
  _pumpAudio() {
    ...
    for (const c of chunks) {
      ...
      this.stats.audioSamples += c.samples.length;
      if (this._audioNode) {
        this._audioNode.port.postMessage({ type: "pcm", samples: c.samples }, [c.samples.buffer]);
      }
      c.free?.();
    }
  }
  ...
  _ensureAudio(sampleRate, channels) {
    ...
    this._audioCtx.audioWorklet.addModule(
      ...
      this._audioNode.port.postMessage({ type: "config", sampleRate, channels });
